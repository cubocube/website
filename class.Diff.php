  ...
  public static function toString($diff, $separator = "\n"){
    ...
    foreach ($diff as $line){
      ...
      switch ($line[1]){
        ...
        case self::DELETED    : 
        {
            $line[0] = preg_replace('/(<img[^>]+\>)/i', '<div class="del-img">$1</div>', $line[0]);
            $tofind = array('<','>', 'specrighttag');
            $torepl = array('</spanspecrighttag<', '><span style="opacity: 0.3;">', '>');
            $addstring = str_replace($tofind, $torepl, $line[0]);
            $addstring = '<span style="opacity: 0.3;">' . $addstring."</span>";
            $string .= $addstring;
            break;
        }
        case self::INSERTED   : 
        {
            $line[0] = preg_replace('/(<img[^>]+\>)/i', '<div class="new-img">$1</div>', $line[0]);
            $tofind = array('<','>', 'specrighttag');
            $torepl = array('</spanspecrighttag<', '><span style="background: #4f4;">', '>');
            $addstring = str_replace($tofind, $torepl, $line[0]);
            $addstring = '<span style="background: #4f4;">' . $addstring."</span>";
            $string .= $addstring;
            break;
        }
      }
