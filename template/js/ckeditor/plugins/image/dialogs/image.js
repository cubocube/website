            ...
            contents: [{
                ...
                elements: [{
                        ...
                        children: [{
                            ...
                            onChange: function () {
                                ...
                                if (C.length > 0) {
                                    B = this.getDialog();
                                    var D = B.originalElement;
                                    if (B.preview) B.preview.removeStyle('display');
                                    if (D) D.setCustomData('isReady', 'false');
                                    ...
                                    if (s) s.setAttribute('src', C);
                                    if (B.preview) B.preview.setAttribute('src', s.$.src);
                                    l(B);
                                } else if (B.preview) {
                    ...
                    children: [{
                        ...
                        width: '100%',
                        height: '100%',
                        style: 'width:100%;height:100%;min-width:500px;min-height:300px',
                        heights: ['100%'],
                        children: [
                            /*{type: 'html',
                             html: '<strong>Upload an Image</strong>'},
                            {type: 'file',
                             id: 'fileUp',
                             onChange: function() {
                                 $.post
                            ...
                            {type: 'html',
                             ...
                             html: '<iframe id="fileFrame" src="image_upload_frame.php" onload="onImageUploadLoad();" style="display:block;width:100%;height:100%" width="100%" height="100%"></iframe>'
                            }]
                        //]
                    //}]
                }]
...
function onImageUploadLoad(document) {
    var ifDoc = $($('#fileFrame')[0].contentWindow.document);
    $('#fileFrame').css('height',
        (ifDoc[0].documentElement.scrollHeight || ifDoc[0].body.scrollHeight) + 'px');
    var path = ifDoc.find('#path');
    if (path.length > 0) {
        var dlg = CKEDITOR.dialog.getCurrent();
        dlg.getContentElement('info', 'txtUrl').setValue(path.text());
        dlg.getContentElement('info', 'txtWidth').setValue(ifDoc.find('#width').text());
        dlg.getContentElement('info', 'txtHeight').setValue(ifDoc.find('#height').text());
        ifDoc.find('#error').hide();
        $('#' + dlg.getButton('ok').domId)[0].click();
    }
}
